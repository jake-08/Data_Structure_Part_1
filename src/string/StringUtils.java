...
//        return result;
        ...
        for (var ch : str.toCharArray()) {
            if (frequencies1.containsKey(ch))
                frequencies1.replace(ch, frequencies1.get(ch) + 1);
            else
                frequencies1.put(ch, 1);
        }
        int max = 0;
        char result = ' ';
        for (var chars : frequencies1.entrySet()) {
            if (chars.getValue() > max) {
                result = chars.getKey();
                max = chars.getValue();
            }
        }
...
//        We have to iterate the input string four times

//        StringBuilder input = new StringBuilder(word);
//        input.reverse();
//        return input.toString().equals(word);

        // this approach we have to iterate only half of the characters - faster
        int left = 0;
